from __future__ import annotations

from typing import Any, TYPE_CHECKING

from .._http import camel_to_snake, format_path, split_request_args

if TYPE_CHECKING:
    from ..client import AsyncVideoGen, VideoGen


class WebhooksResource:
    def __init__(self, client: VideoGen) -> None:
        self._root = client
        self._client = client._http

    # @sdk-operation listWebhookEndpoints
    def list_webhook_endpoints(self, *args: Any, **kwargs: Any) -> Any:
        if args:
            raise TypeError(f'list_webhook_endpoints() does not take positional arguments')
        path_values, query_values, body, cancel_event = split_request_args(
            kwargs,
            path_params=[],
            query_params=['limit', 'cursor'],
        )
        path = format_path('/v1/webhooks/endpoints', path_values)
        return self._client.request(
            method='GET',
            path=path,
            query=query_values or None,
            body=body,
            cancel_event=cancel_event,
        )

    # @sdk-operation createWebhookEndpoint
    def create_webhook_endpoint(self, *args: Any, **kwargs: Any) -> Any:
        if args:
            raise TypeError(f'create_webhook_endpoint() does not take positional arguments')
        path_values, query_values, body, cancel_event = split_request_args(
            kwargs,
            path_params=[],
            query_params=[],
        )
        path = format_path('/v1/webhooks/endpoints', path_values)
        return self._client.request(
            method='POST',
            path=path,
            query=query_values or None,
            body=body if body is not None else {},
            cancel_event=cancel_event,
        )

    # @sdk-operation deleteWebhookEndpoint
    def delete_webhook_endpoint(self, *args: Any, **kwargs: Any) -> Any:
        path_param_names = ['endpoint_id']
        if args:
            if len(args) > len(path_param_names):
                raise TypeError(f'delete_webhook_endpoint() takes at most {len(path_param_names)} positional argument(s)')
            for name, value in zip(path_param_names, args):
                if name in kwargs:
                    raise TypeError(f"{name} specified twice")
                kwargs[name] = value
        path_values, query_values, body, cancel_event = split_request_args(
            kwargs,
            path_params=['endpointId'],
            query_params=[],
        )
        for required in ['endpointId']:
            if required not in path_values:
                raise TypeError(f"missing required argument: {camel_to_snake(required)}")
        path = format_path('/v1/webhooks/endpoints/{endpointId}', path_values)
        return self._client.request(
            method='DELETE',
            path=path,
            query=query_values or None,
            body=body,
            cancel_event=cancel_event,
        )


class AsyncWebhooksResource:
    def __init__(self, client: AsyncVideoGen) -> None:
        self._root = client
        self._client = client._http

    # @sdk-operation listWebhookEndpoints
    async def list_webhook_endpoints(self, *args: Any, **kwargs: Any) -> Any:
        if args:
            raise TypeError(f'list_webhook_endpoints() does not take positional arguments')
        path_values, query_values, body, cancel_event = split_request_args(
            kwargs,
            path_params=[],
            query_params=['limit', 'cursor'],
        )
        path = format_path('/v1/webhooks/endpoints', path_values)
        return await self._client.request(
            method='GET',
            path=path,
            query=query_values or None,
            body=body,
            cancel_event=cancel_event,
        )

    # @sdk-operation createWebhookEndpoint
    async def create_webhook_endpoint(self, *args: Any, **kwargs: Any) -> Any:
        if args:
            raise TypeError(f'create_webhook_endpoint() does not take positional arguments')
        path_values, query_values, body, cancel_event = split_request_args(
            kwargs,
            path_params=[],
            query_params=[],
        )
        path = format_path('/v1/webhooks/endpoints', path_values)
        return await self._client.request(
            method='POST',
            path=path,
            query=query_values or None,
            body=body if body is not None else {},
            cancel_event=cancel_event,
        )

    # @sdk-operation deleteWebhookEndpoint
    async def delete_webhook_endpoint(self, *args: Any, **kwargs: Any) -> Any:
        path_param_names = ['endpoint_id']
        if args:
            if len(args) > len(path_param_names):
                raise TypeError(f'delete_webhook_endpoint() takes at most {len(path_param_names)} positional argument(s)')
            for name, value in zip(path_param_names, args):
                if name in kwargs:
                    raise TypeError(f"{name} specified twice")
                kwargs[name] = value
        path_values, query_values, body, cancel_event = split_request_args(
            kwargs,
            path_params=['endpointId'],
            query_params=[],
        )
        for required in ['endpointId']:
            if required not in path_values:
                raise TypeError(f"missing required argument: {camel_to_snake(required)}")
        path = format_path('/v1/webhooks/endpoints/{endpointId}', path_values)
        return await self._client.request(
            method='DELETE',
            path=path,
            query=query_values or None,
            body=body,
            cancel_event=cancel_event,
        )
