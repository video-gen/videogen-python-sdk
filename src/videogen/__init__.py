# This file was auto-generated by Fern from our API Definition.

# isort: skip_file

import typing
from importlib import import_module

if typing.TYPE_CHECKING:
    from .types import (
        ApiError,
        ApiErrorRequirement,
        AspectRatio,
        AssistantAction,
        AssistantActionKind,
        AssistantTurnResponse,
        AssistantTurnStatus,
        AssistantWorkflowSuggestion,
        AvatarPresenter,
        AvatarPresenterDisplayableGender,
        AvatarPresenterListResponse,
        Entity,
        EntityActorConfig,
        EntityArchiveResponse,
        EntityEntityType,
        EntityReference,
        ExecutedTool,
        ExecutedToolStatus,
        ExportDeliveryDestination,
        ExportDeliveryDestinationType,
        ExportProjectQuality,
        ExportProjectResponse,
        FileAnalysisMetadata,
        FileAnalysisMetadataAnalysisLoadingState,
        FileSource,
        FileSourceStatus,
        FileUploadResponse,
        FileUploadWebhookEventName,
        FileUploadWebhookPayload,
        GenerateStoryboardScene,
        GenerateTextResponse,
        GetFilesResponse,
        ImageAssetRequest,
        ImageQuality,
        Language,
        LanguageListResponse,
        ListEntitiesResponse,
        ListProjectsResponse,
        ListRemixActionsResponse,
        MeResponse,
        ProjectExport,
        ProjectExportStatus,
        ProjectResponse,
        ProjectResponseStatus,
        PronunciationReplacement,
        RemixAction,
        RemixActionAddTransitions,
        RemixActionChangeNarrator,
        RemixActionCleanUpTranscript,
        RemixActionConvertImagesToVideos,
        RemixActionDisableCaptions,
        RemixActionEditWithAgent,
        RemixActionEditWithAgentMode,
        RemixActionEnableCaptions,
        RemixActionGenerateMusic,
        RemixActionRegenerateImages,
        RemixActionRegenerateImagesModelMode,
        RemixActionResizeProject,
        RemixActionRun,
        RemixActionSetBackgroundMusic,
        RemixActionSetLogo,
        RemixActionSetLogoPosition,
        RemixActionShuffleStockVisuals,
        RemixActionStatus,
        RemixActionTranslateProject,
        RemixActionType,
        RemixActionUpscaleAssets,
        RemixAction_AddTransitions,
        RemixAction_ChangeNarrator,
        RemixAction_CleanUpTranscript,
        RemixAction_ConvertImagesToVideos,
        RemixAction_DisableCaptions,
        RemixAction_EditWithAgent,
        RemixAction_EnableCaptions,
        RemixAction_GenerateMusic,
        RemixAction_RegenerateImages,
        RemixAction_ResizeProject,
        RemixAction_SetBackgroundMusic,
        RemixAction_SetLogo,
        RemixAction_ShuffleStockVisuals,
        RemixAction_TranslateProject,
        RemixAction_UpscaleAssets,
        RemixProjectResponse,
        RemixTransitionStyle,
        SceneGeneration,
        SceneGenerationType,
        ScriptToVideoRequest,
        ScriptToVideoRequestQuality,
        SearchFilesResponse,
        SearchFilesResult,
        SlideshowToVideoRequest,
        StartToolExecutionResponse,
        StartWorkflowRunResponse,
        StorageFile,
        StorageFileScope,
        StorageFileType,
        StoryboardToVideoRequest,
        StoryboardToVideoRequestQuality,
        ToolExecutionListResponse,
        ToolExecutionWebhookEventName,
        ToolExecutionWebhookPayload,
        ToolSuccessResult,
        ToolSuccessResultType,
        TtsVoice,
        TtsVoiceDisplayGender,
        TtsVoiceListResponse,
        VideoAssetRequest,
        VideoQuality,
        VisualPacing,
        VoiceoverToVideoRequest,
        VoiceoverToVideoRequestQuality,
        WatermarkMode,
        WebhookEndpoint,
        WebhookEndpointListResponse,
        WebhookEventName,
        WorkflowCaptionBackgroundStyle,
        WorkflowCaptionBackgroundStyleType,
        WorkflowCaptionStyle,
        WorkflowCaptionStyleTextJustification,
        WorkflowCaptionStyleVerticalAlignment,
        WorkflowRgbColor,
        WorkflowRun,
        WorkflowRunListResponse,
        WorkflowRunStatus,
        WorkflowRunWebhookEventName,
        WorkflowRunWebhookPayload,
        WorkflowType,
        WorkflowVisualStyle,
        WorkflowVisualStyleType,
    )
    from . import account, assistant, entities, files, projects, resources, text, tools, webhooks, workflows
    from ._default_clients import DefaultAioHttpClient, DefaultAsyncHttpxClient
    from .assistant import ActOnAssistantActionRequestDecision
    from .async_download_file import async_download_file
    from .async_get_hydrated_file import async_get_hydrated_file
    from .async_poll_executed_tool import async_poll_executed_tool
    from .async_poll_project_export import async_poll_project_export
    from .async_poll_public_preview import (
        async_poll_public_embed_playback_id,
        async_poll_public_preview,
        async_poll_public_preview_url,
    )
    from .async_poll_workflow_run import async_poll_workflow_run
    from .async_upload_file import async_upload_file
    from .client import AsyncVideoGenApi, VideoGenApi
    from .download_file import download_file
    from .entities import CreateEntityRequestEntityType, ListEntitiesRequestEntityType
    from .environment import VideoGenApiEnvironment
    from .files import CreateFileUploadRequestType
    from .get_hydrated_file import get_hydrated_file
    from .poll_executed_tool import poll_executed_tool
    from .poll_project_export import poll_project_export
    from .poll_public_preview import (
        PUBLIC_PREVIEW_NOT_ENABLED_ERROR_MESSAGE,
        poll_public_embed_playback_id,
        poll_public_preview,
        poll_public_preview_url,
    )
    from .poll_workflow_run import poll_workflow_run
    from .text import GenerateTextRequestQuality
    from .tools import GenerateImageRequestQuality, GenerateVideoClipRequestQuality
    from .upload_file import upload_file
    from .verify_webhook_signature import WebhookEvent, verify_webhook_signature
    from .version import __version__
_dynamic_imports: typing.Dict[str, str] = {
    "ActOnAssistantActionRequestDecision": ".assistant",
    "ApiError": ".types",
    "ApiErrorRequirement": ".types",
    "AspectRatio": ".types",
    "AssistantAction": ".types",
    "AssistantActionKind": ".types",
    "AssistantTurnResponse": ".types",
    "AssistantTurnStatus": ".types",
    "AssistantWorkflowSuggestion": ".types",
    "AsyncVideoGenApi": ".client",
    "AvatarPresenter": ".types",
    "AvatarPresenterDisplayableGender": ".types",
    "AvatarPresenterListResponse": ".types",
    "CreateEntityRequestEntityType": ".entities",
    "CreateFileUploadRequestType": ".files",
    "DefaultAioHttpClient": "._default_clients",
    "DefaultAsyncHttpxClient": "._default_clients",
    "Entity": ".types",
    "EntityActorConfig": ".types",
    "EntityArchiveResponse": ".types",
    "EntityEntityType": ".types",
    "EntityReference": ".types",
    "ExecutedTool": ".types",
    "ExecutedToolStatus": ".types",
    "ExportDeliveryDestination": ".types",
    "ExportDeliveryDestinationType": ".types",
    "ExportProjectQuality": ".types",
    "ExportProjectResponse": ".types",
    "FileAnalysisMetadata": ".types",
    "FileAnalysisMetadataAnalysisLoadingState": ".types",
    "FileSource": ".types",
    "FileSourceStatus": ".types",
    "FileUploadResponse": ".types",
    "FileUploadWebhookEventName": ".types",
    "FileUploadWebhookPayload": ".types",
    "GenerateImageRequestQuality": ".tools",
    "GenerateStoryboardScene": ".types",
    "GenerateTextRequestQuality": ".text",
    "GenerateTextResponse": ".types",
    "GenerateVideoClipRequestQuality": ".tools",
    "GetFilesResponse": ".types",
    "ImageAssetRequest": ".types",
    "ImageQuality": ".types",
    "Language": ".types",
    "LanguageListResponse": ".types",
    "ListEntitiesRequestEntityType": ".entities",
    "ListEntitiesResponse": ".types",
    "ListProjectsResponse": ".types",
    "ListRemixActionsResponse": ".types",
    "MeResponse": ".types",
    "PUBLIC_PREVIEW_NOT_ENABLED_ERROR_MESSAGE": ".poll_public_preview",
    "ProjectExport": ".types",
    "ProjectExportStatus": ".types",
    "ProjectResponse": ".types",
    "ProjectResponseStatus": ".types",
    "PronunciationReplacement": ".types",
    "RemixAction": ".types",
    "RemixActionAddTransitions": ".types",
    "RemixActionChangeNarrator": ".types",
    "RemixActionCleanUpTranscript": ".types",
    "RemixActionConvertImagesToVideos": ".types",
    "RemixActionDisableCaptions": ".types",
    "RemixActionEditWithAgent": ".types",
    "RemixActionEditWithAgentMode": ".types",
    "RemixActionEnableCaptions": ".types",
    "RemixActionGenerateMusic": ".types",
    "RemixActionRegenerateImages": ".types",
    "RemixActionRegenerateImagesModelMode": ".types",
    "RemixActionResizeProject": ".types",
    "RemixActionRun": ".types",
    "RemixActionSetBackgroundMusic": ".types",
    "RemixActionSetLogo": ".types",
    "RemixActionSetLogoPosition": ".types",
    "RemixActionShuffleStockVisuals": ".types",
    "RemixActionStatus": ".types",
    "RemixActionTranslateProject": ".types",
    "RemixActionType": ".types",
    "RemixActionUpscaleAssets": ".types",
    "RemixAction_AddTransitions": ".types",
    "RemixAction_ChangeNarrator": ".types",
    "RemixAction_CleanUpTranscript": ".types",
    "RemixAction_ConvertImagesToVideos": ".types",
    "RemixAction_DisableCaptions": ".types",
    "RemixAction_EditWithAgent": ".types",
    "RemixAction_EnableCaptions": ".types",
    "RemixAction_GenerateMusic": ".types",
    "RemixAction_RegenerateImages": ".types",
    "RemixAction_ResizeProject": ".types",
    "RemixAction_SetBackgroundMusic": ".types",
    "RemixAction_SetLogo": ".types",
    "RemixAction_ShuffleStockVisuals": ".types",
    "RemixAction_TranslateProject": ".types",
    "RemixAction_UpscaleAssets": ".types",
    "RemixProjectResponse": ".types",
    "RemixTransitionStyle": ".types",
    "SceneGeneration": ".types",
    "SceneGenerationType": ".types",
    "ScriptToVideoRequest": ".types",
    "ScriptToVideoRequestQuality": ".types",
    "SearchFilesResponse": ".types",
    "SearchFilesResult": ".types",
    "SlideshowToVideoRequest": ".types",
    "StartToolExecutionResponse": ".types",
    "StartWorkflowRunResponse": ".types",
    "StorageFile": ".types",
    "StorageFileScope": ".types",
    "StorageFileType": ".types",
    "StoryboardToVideoRequest": ".types",
    "StoryboardToVideoRequestQuality": ".types",
    "ToolExecutionListResponse": ".types",
    "ToolExecutionWebhookEventName": ".types",
    "ToolExecutionWebhookPayload": ".types",
    "ToolSuccessResult": ".types",
    "ToolSuccessResultType": ".types",
    "TtsVoice": ".types",
    "TtsVoiceDisplayGender": ".types",
    "TtsVoiceListResponse": ".types",
    "VideoAssetRequest": ".types",
    "VideoGenApi": ".client",
    "VideoGenApiEnvironment": ".environment",
    "VideoQuality": ".types",
    "VisualPacing": ".types",
    "VoiceoverToVideoRequest": ".types",
    "VoiceoverToVideoRequestQuality": ".types",
    "WatermarkMode": ".types",
    "WebhookEndpoint": ".types",
    "WebhookEndpointListResponse": ".types",
    "WebhookEvent": ".verify_webhook_signature",
    "WebhookEventName": ".types",
    "WorkflowCaptionBackgroundStyle": ".types",
    "WorkflowCaptionBackgroundStyleType": ".types",
    "WorkflowCaptionStyle": ".types",
    "WorkflowCaptionStyleTextJustification": ".types",
    "WorkflowCaptionStyleVerticalAlignment": ".types",
    "WorkflowRgbColor": ".types",
    "WorkflowRun": ".types",
    "WorkflowRunListResponse": ".types",
    "WorkflowRunStatus": ".types",
    "WorkflowRunWebhookEventName": ".types",
    "WorkflowRunWebhookPayload": ".types",
    "WorkflowType": ".types",
    "WorkflowVisualStyle": ".types",
    "WorkflowVisualStyleType": ".types",
    "__version__": ".version",
    "account": ".account",
    "assistant": ".assistant",
    "async_download_file": ".async_download_file",
    "async_get_hydrated_file": ".async_get_hydrated_file",
    "async_poll_executed_tool": ".async_poll_executed_tool",
    "async_poll_project_export": ".async_poll_project_export",
    "async_poll_public_embed_playback_id": ".async_poll_public_preview",
    "async_poll_public_preview": ".async_poll_public_preview",
    "async_poll_public_preview_url": ".async_poll_public_preview",
    "async_poll_workflow_run": ".async_poll_workflow_run",
    "async_upload_file": ".async_upload_file",
    "download_file": ".download_file",
    "entities": ".entities",
    "files": ".files",
    "get_hydrated_file": ".get_hydrated_file",
    "poll_executed_tool": ".poll_executed_tool",
    "poll_project_export": ".poll_project_export",
    "poll_public_embed_playback_id": ".poll_public_preview",
    "poll_public_preview": ".poll_public_preview",
    "poll_public_preview_url": ".poll_public_preview",
    "poll_workflow_run": ".poll_workflow_run",
    "projects": ".projects",
    "resources": ".resources",
    "text": ".text",
    "tools": ".tools",
    "upload_file": ".upload_file",
    "verify_webhook_signature": ".verify_webhook_signature",
    "webhooks": ".webhooks",
    "workflows": ".workflows",
}


def __getattr__(attr_name: str) -> typing.Any:
    module_name = _dynamic_imports.get(attr_name)
    if module_name is None:
        raise AttributeError(f"No {attr_name} found in _dynamic_imports for module name -> {__name__}")
    try:
        module = import_module(module_name, __package__)
        if module_name == f".{attr_name}":
            return module
        else:
            return getattr(module, attr_name)
    except ImportError as e:
        raise ImportError(f"Failed to import {attr_name} from {module_name}: {e}") from e
    except AttributeError as e:
        raise AttributeError(f"Failed to get {attr_name} from {module_name}: {e}") from e


def __dir__():
    lazy_attrs = list(_dynamic_imports.keys())
    return sorted(lazy_attrs)


__all__ = [
    "ActOnAssistantActionRequestDecision",
    "ApiError",
    "ApiErrorRequirement",
    "AspectRatio",
    "AssistantAction",
    "AssistantActionKind",
    "AssistantTurnResponse",
    "AssistantTurnStatus",
    "AssistantWorkflowSuggestion",
    "AsyncVideoGenApi",
    "AvatarPresenter",
    "AvatarPresenterDisplayableGender",
    "AvatarPresenterListResponse",
    "CreateEntityRequestEntityType",
    "CreateFileUploadRequestType",
    "DefaultAioHttpClient",
    "DefaultAsyncHttpxClient",
    "Entity",
    "EntityActorConfig",
    "EntityArchiveResponse",
    "EntityEntityType",
    "EntityReference",
    "ExecutedTool",
    "ExecutedToolStatus",
    "ExportDeliveryDestination",
    "ExportDeliveryDestinationType",
    "ExportProjectQuality",
    "ExportProjectResponse",
    "FileAnalysisMetadata",
    "FileAnalysisMetadataAnalysisLoadingState",
    "FileSource",
    "FileSourceStatus",
    "FileUploadResponse",
    "FileUploadWebhookEventName",
    "FileUploadWebhookPayload",
    "GenerateImageRequestQuality",
    "GenerateStoryboardScene",
    "GenerateTextRequestQuality",
    "GenerateTextResponse",
    "GenerateVideoClipRequestQuality",
    "GetFilesResponse",
    "ImageAssetRequest",
    "ImageQuality",
    "Language",
    "LanguageListResponse",
    "ListEntitiesRequestEntityType",
    "ListEntitiesResponse",
    "ListProjectsResponse",
    "ListRemixActionsResponse",
    "MeResponse",
    "PUBLIC_PREVIEW_NOT_ENABLED_ERROR_MESSAGE",
    "ProjectExport",
    "ProjectExportStatus",
    "ProjectResponse",
    "ProjectResponseStatus",
    "PronunciationReplacement",
    "RemixAction",
    "RemixActionAddTransitions",
    "RemixActionChangeNarrator",
    "RemixActionCleanUpTranscript",
    "RemixActionConvertImagesToVideos",
    "RemixActionDisableCaptions",
    "RemixActionEditWithAgent",
    "RemixActionEditWithAgentMode",
    "RemixActionEnableCaptions",
    "RemixActionGenerateMusic",
    "RemixActionRegenerateImages",
    "RemixActionRegenerateImagesModelMode",
    "RemixActionResizeProject",
    "RemixActionRun",
    "RemixActionSetBackgroundMusic",
    "RemixActionSetLogo",
    "RemixActionSetLogoPosition",
    "RemixActionShuffleStockVisuals",
    "RemixActionStatus",
    "RemixActionTranslateProject",
    "RemixActionType",
    "RemixActionUpscaleAssets",
    "RemixAction_AddTransitions",
    "RemixAction_ChangeNarrator",
    "RemixAction_CleanUpTranscript",
    "RemixAction_ConvertImagesToVideos",
    "RemixAction_DisableCaptions",
    "RemixAction_EditWithAgent",
    "RemixAction_EnableCaptions",
    "RemixAction_GenerateMusic",
    "RemixAction_RegenerateImages",
    "RemixAction_ResizeProject",
    "RemixAction_SetBackgroundMusic",
    "RemixAction_SetLogo",
    "RemixAction_ShuffleStockVisuals",
    "RemixAction_TranslateProject",
    "RemixAction_UpscaleAssets",
    "RemixProjectResponse",
    "RemixTransitionStyle",
    "SceneGeneration",
    "SceneGenerationType",
    "ScriptToVideoRequest",
    "ScriptToVideoRequestQuality",
    "SearchFilesResponse",
    "SearchFilesResult",
    "SlideshowToVideoRequest",
    "StartToolExecutionResponse",
    "StartWorkflowRunResponse",
    "StorageFile",
    "StorageFileScope",
    "StorageFileType",
    "StoryboardToVideoRequest",
    "StoryboardToVideoRequestQuality",
    "ToolExecutionListResponse",
    "ToolExecutionWebhookEventName",
    "ToolExecutionWebhookPayload",
    "ToolSuccessResult",
    "ToolSuccessResultType",
    "TtsVoice",
    "TtsVoiceDisplayGender",
    "TtsVoiceListResponse",
    "VideoAssetRequest",
    "VideoGenApi",
    "VideoGenApiEnvironment",
    "VideoQuality",
    "VisualPacing",
    "VoiceoverToVideoRequest",
    "VoiceoverToVideoRequestQuality",
    "WatermarkMode",
    "WebhookEndpoint",
    "WebhookEndpointListResponse",
    "WebhookEvent",
    "WebhookEventName",
    "WorkflowCaptionBackgroundStyle",
    "WorkflowCaptionBackgroundStyleType",
    "WorkflowCaptionStyle",
    "WorkflowCaptionStyleTextJustification",
    "WorkflowCaptionStyleVerticalAlignment",
    "WorkflowRgbColor",
    "WorkflowRun",
    "WorkflowRunListResponse",
    "WorkflowRunStatus",
    "WorkflowRunWebhookEventName",
    "WorkflowRunWebhookPayload",
    "WorkflowType",
    "WorkflowVisualStyle",
    "WorkflowVisualStyleType",
    "__version__",
    "account",
    "assistant",
    "async_download_file",
    "async_get_hydrated_file",
    "async_poll_executed_tool",
    "async_poll_project_export",
    "async_poll_public_embed_playback_id",
    "async_poll_public_preview",
    "async_poll_public_preview_url",
    "async_poll_workflow_run",
    "async_upload_file",
    "download_file",
    "entities",
    "files",
    "get_hydrated_file",
    "poll_executed_tool",
    "poll_project_export",
    "poll_public_embed_playback_id",
    "poll_public_preview",
    "poll_public_preview_url",
    "poll_workflow_run",
    "projects",
    "resources",
    "text",
    "tools",
    "upload_file",
    "verify_webhook_signature",
    "webhooks",
    "workflows",
]
